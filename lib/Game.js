...
class Game {
  constructor(gameSize) {
    ...
    this.gamePieces = ([new Player(this.gameSize.width / 2, this.gameSize.height - 15, 15, 15)]).concat([new Platform ((Math.floor(Math.random() * 500)), this.gameSize.height - 100, 50, 15), 
                                                                                                        new Platform ((Math.floor(Math.random() * 500)), this.gameSize.height - 150, 50, 15), 
                                                                                                        new Platform ((Math.floor(Math.random() * 500)), this.gameSize.height - 200, 50, 15),
                                                                                                        new Platform ((Math.floor(Math.random() * 500)), this.gameSize.height - 250, 50, 15),
                                                                                                        new Platform ((Math.floor(Math.random() * 500)), this.gameSize.height - 300, 50, 15)])
  }
  ...
  collisionDetection() { //consider changing .center back to .position\
    ...
    const colliding = this.gamePieces.filter( platform => {
      if (player.position.x + player.size.w >= platform.position.x && 
        ...
        this.gamePieces.forEach( platform => platform.accelerate(7, -0.2));
